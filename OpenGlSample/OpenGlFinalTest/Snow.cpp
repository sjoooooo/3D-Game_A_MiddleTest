...
void Snow::init()
{
	...
	filemgr->loadOBJs(this, "sphere.obj", "white.bmp", "20161614_vs.shader", "20161614_fs.shader");
	...
	if (random == 1)
	{
		this->setPosition(randomPos, 60.0f, 0.0f);
	}
	else if (random == 2)
	{
		this->setPosition(-(randomPos), 20.0f, 0.0f);
	}
}
...
void Snow::setRot(float speed, float x, float y, float z)
{
	rotSpeed = speed;
	rotVec = glm::vec3(x, y, z);
}

void Snow::setScale(float x, float y, float z)
{
...
void Snow::setCameraPos(float x, float y, float z)
{
	cameraPos = glm::vec3(-x, -y, -z);
}

void Snow::AddChild(CompositeObject* addObj)
{
